...
import com.spacester.tweetster.activity.AddActivity;
import com.spacester.tweetster.activity.StoryViewActivity;
        ...
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (!modelUser.getPhoto().isEmpty()){
                    Glide.with(context).asBitmap().load(Objects.requireNonNull(modelUser).getPhoto()).centerCrop().into(viewHolder.story_photo);
                }else {
                    Glide.with(context).asBitmap().load(R.drawable.avatar).centerCrop().into(viewHolder.story_photo);
                }
                if (pos != 0){
                    if (!modelUser.getPhoto().isEmpty()){
                        Glide.with(context).asBitmap().load(modelUser.getPhoto()).centerCrop().into(viewHolder.story_photo_seen);
                    }else {
